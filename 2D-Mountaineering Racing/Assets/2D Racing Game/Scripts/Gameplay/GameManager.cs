		...
		AdMgr.ShowVideoAd("192if3b93qo6991ed0",
			(bol) => {
				if (bol)
				{
					...
					GameObject carObj = GameObject.FindObjectOfType<CarController>().gameObject;
					carObj.transform.localRotation = Quaternion.Euler(0, 0, 0);
					TotalFuel += 50f;
					if(TotalFuel>100)
                    {
						TotalFuel = 100;
